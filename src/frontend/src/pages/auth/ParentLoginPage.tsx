...
export default function ParentLoginPage() {
  ...
  useEffect(() => {
    if (identity && profile && profile.role === UserRole.parent) {
      authLogin(profile.phoneNumber, UserRole.parent);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profile.role !== UserRole.parent) {
        setErrors({ submit: 'This account is not registered as a parent.' });
        setIsSubmitting(false);
        return;
      }
    ...
    <div className="container mx-auto px-4 py-12 flex items-center justify-center min-h-[60vh]">
      <Card className="max-w-md w-full">
        <CardHeader className="text-center">
          ...
          <CardDescription>Sign in to view your child's attendance</CardDescription>
        </CardHeader>
        <CardContent>
